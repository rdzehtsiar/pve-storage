...
sub _find_previous_backup {
    my ($self, $vmid, $dev) = @_;

    my $glob = "$self->{backup_path}/demo-qemu-$vmid-*-${dev}.raw";
    my @files = sort glob($glob);
    return $files[-1] if @files;
    return undef;
}
...
sub backup_vm_query_incremental {
    ...
    my %use = map { $_ => 'use' } keys %$volumes;
    $self->_log(
        'backup_vm_query_incremental vmid=' . $vmid . ' devices=' . join(',', keys %use)
    );
...
sub backup_vm {
    ...
    foreach my $dev (sort keys %$volumes) {
        ...
        my $next_dirty = $volumes->{$dev}->{'next-dirty-region'};
        my $size = $volumes->{$dev}->{size};
        ...
        my $prev = $self->_find_previous_backup($vmid, $name);
        if ($prev) {
            $self->_log("using previous backup $prev as base for $dest");
            File::Copy::copy($prev, $dest) or die "copy $prev to $dest failed: $!";
        } else {
            open(my $tmp, '>', $dest) or die "unable to create $dest: $!";
            truncate($tmp, $size) if defined $size;
            close($tmp);
        }
        ...
        my $buf;
        if ($next_dirty) {
            while (my ($off, $len) = $next_dirty->()) {
                $self->_log("dirty region $dev off=$off len=$len");
                sysseek($fh, $off, 0);
                sysseek($out, $off, 0);
                my $remain = $len;
                while ($remain > 0) {
                    my $chunk = $remain > 8192 ? 8192 : $remain;
                    my $read = sysread($fh, $buf, $chunk);
                    die "read failed: $!" if !defined $read;
                    last if $read == 0;
                    my $written = syswrite($out, $buf, $read);
                    die "write failed: $!" if !defined $written || $written != $read;
                    $remain -= $read;
                }
            }
        } else {
